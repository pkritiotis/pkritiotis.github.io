...
For most of our time, we, software engineers are involved in this whole process after the business requirements phase with the responsibility to design, implement, test and deploy applications.

Between the handover of the requirements from the product team to the engineers and the initiation of the design there is a short period in which engineers work with the product team to clarify any ambiguous requirements, talk about acceptance criteria and agree on specific business specs. 

This phase which in this article I refer to as **Technical Requirement Analysis**, is one of the most important phases in the lifecycle. 

Below we will explore this phase in detail and provide a well-defined approach on how to structure through this phase to minimize unclarity and future inconvenience.


# Importance
The Technical Analysis of Requirements phase is important for many reasons:
1. It **ensures that engineering and product are aligned** on the goal and the problem this feature is trying to solve. While the product team has gathers the requirements from the stakeholders and are aligned with business, it is also important that the defined requirements are well understood by the engineering too. This will ensure that the end-goal is the same from all teams' perpectives. 
2. It **allows engineers to give their technical perspective** on the requirements. Firstly, some features can be technically infeasible. This is usually prevented by having some technical presence when the product defines the business requirements. However, even for feasible issues, depending on the interpretation of the requirements, a system can have simple or complicated technical challenges. And a small tweak that is not so important for the product can decrease the complexity and consequently the delivery time of the feature.
3. Product sees requirements from the customer perspective. Engineers however can identify collateral technical consequences, future codebase quality issues and express their concerns. Depending on the priorities and the long term vision, this input from engineers might change the course of the feature and alter some of the requirements.

# A simple but solid Technical Requirement Analysis approach
In my experience working on new features, I have not seen a standard way to approach this phase. However, through the years I have personally used a few tools that have been incrementally optimizing my approach on this . In this sesction I will be sharing this approach.
...
Once all the requirement analysis is done, with all edge cases documented with possible options and maybe the recommended approach, the product team can review the new infrormation.
...
Having the requirements well-defined with all edge cases the engineering team can proceed with the technical analysis.
...
This is particularly useful when working with code bases using [Clean Architecture]().
